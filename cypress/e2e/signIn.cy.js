...
describe('Sign In page', () => {
  ...
  it(`shouldn't provide the ability to login with invalid credentials`, () => {
    cy.get('.navbar a').contains('Sign in').click();
    cy.get('input[type="email"]').type('kredkivillain@kredki.com');
    cy.get('input[type ="password"]').type('kredkibadguy');
    cy.get('button[type="submit"]').click();
    cy.get('ul').should('contain', 'email or password is invalid');
  });
  it('should provide an ability to log out after succesfull login', () => {
    ...
    cy.get('.navbar').should('not.contain', 'kredki');
  });
});
